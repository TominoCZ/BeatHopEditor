...
namespace BeatHopEditor.Gui
{
	class GuiScreenLoadCreate : GuiScreen
	{
		...
		private readonly GuiButton _importButton;
		...
		public GuiScreenLoadCreate() : base(0, 0, EditorWindow.Instance.ClientSize.Width, EditorWindow.Instance.ClientSize.Height)
		{
			...
			_importButton = new GuiButton(3, 0, 0, 192, 64, "PASTE SS MAP");
				...
				case 3:
					{
						...
						if (!string.IsNullOrWhiteSpace(clipboard))
						{
							EditorWindow.Instance.LoadMap(clipboard, true);
						}
						break;
					}
			}
		...
		public sealed override void OnResize(Size size)
		{
			...
			var width = _pasteButton.ClientRectangle.Width;

			_pasteButton.ClientRectangle.Location = new PointF((int)(size.Width / 2f - width - 10), 512 - 64 - 20);
			_importButton.ClientRectangle.Location = new PointF((int)(size.Width / 2f + 10), 512 - 64 - 20);
