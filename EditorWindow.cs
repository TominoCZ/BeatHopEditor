...
namespace BeatHopEditor
{
	class EditorWindow : GameWindow
	{
		...
		public readonly Dictionary<Key, int> KeyMapping = new Dictionary<Key, int> {
			{ Key.Q, 0 },
			{ Key.W, 1 },
			{ Key.E, 2 },
			{ Key.R, 3 },
		...
		protected override void OnKeyDown(KeyboardKeyEventArgs e)
		{
			...
			if (GuiScreen is GuiScreenEditor editor)
			{
				...
				if (!e.Control)
				{
					if (KeyMapping.TryGetValue(e.Key, out int posX))
					{
						var note = new Note(posX, editor.Lane,
		...
		public bool LoadMap(string data, bool isSoundSpace = false)
		{
			Notes.Clear();
		...
		public string ParseData()
		{
			...
			for (int i = 0; i < Notes.Count; i++)
			{
				...
				long diff = note.Ms - lastMs;
				long offset = lastMs == 0 ? diff : diff - lastDiff;
